...
select   salesman_id, count(car_id) as cars_sold 
  from    sales 
  group   by salesman_id 

# 3. What is the total revenue generated by each salesperson?
select         salesman_id, sum(s.car_id * cost_$) as Revenue
  from         sales s 
  left join    cars c on s.car_id = c.car_id 
  group by     salesman_id

# 4. What are the details of the cars sold by each salesperson?
select   salesman_id, c.* 
  from   sales s 
  left join   cars c on s.car_id = c.car_id 
  order by   salesman_id

# 5. What is the total revenue generated by each car type?
select   c.type, sum((s.car_id * cost_$)) as Revenue_Generated 
  from   sales s 
  left join   cars c on s.car_id = c.car_id 
  group by   c.type
...
select   c.style, sum((s.car_id * cost_$)) as Revenue_Generated 
  from   sales s 
  left join   cars c on s.car_id = c.car_id 
...
select   name,age 
  ...
  group by   name, age
  order by   sum((s.car_id * c.cost_$)) Desc limit 1 
